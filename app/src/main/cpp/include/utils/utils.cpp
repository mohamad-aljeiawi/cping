...
namespace Utils {
    ...
    void control_frame_rate(float target_fps) {
        ...
        if (target_fps > 0.0f) {
            auto target_frame_time = std::chrono::duration<float, std::micro>(
                    1'000'000.0f / target_fps);
    ...
    float calculateTextSize(float distance, float minDistance, float maxDistance, float minSize,
                            float maxSize, float exponent) {
    ...
    void add_text_center(ImDrawList *draw_list, std::string text, float size, ImVec2 position,
                         ImU32 color, bool center, float gap) {
        if (text.empty())
            return;
    ...
    void advanced_health_bar(ImDrawList *draw_list, float x, float y, float width, float height,
                             float health, float max_health, ImU32 color, ImU32 background_color,
                             float distance, int teamId) {
        ...
        if (healthPercentage > 0.0f) {
            ...
            draw_list->AddRectFilled(
                    ...
                    ImVec2(barPos.x + filledWidth - 1.0f * scale,
                           barPos.y + barHeight - 1.0f * scale),
                    healthColor,
                    1.5f * scale);
        }
