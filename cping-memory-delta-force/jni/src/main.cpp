...
void socket_thread()
{
    ...
    while (is_running.load(std::memory_order_relaxed))
    {
        ...
        if (socket_connected)
        {
            ...
            aim_is_aiming.store(menu_settings.combat_is_aim);
...
void aimbot_thread()
{
    ...
    while (is_running.load(std::memory_order_relaxed))
    {
        ...
        if (!aim_is_aiming.load() || !aim_is_weapon_firing.load())
        {
            if (is_touching)
            {
        ...
        {
            float t = powf((float)(i + 1) / max_iterations, 0.7f);
            float ix = start_x + t * (end_x - start_x);
            float iy = start_y + t * (end_y - start_y);

            if (fabsf(ix - tx) > aim_touch_radius.load() || fabsf(iy - ty) > aim_touch_radius.load())
            {
                TouchInput::sendTouchUp();
                ...
                swipe_dx = (selected_target.X - cx) * sensitivity;
                swipe_dy = (selected_target.Y - cy) * sensitivity;
                end_x = start_x + swipe_dx;
                end_y = start_y + swipe_dy;
            ...
            std::this_thread::sleep_for(std::chrono::microseconds((int)(aim_swipe_duration.load() * 1000 / max_iterations)));
        }
...
int main(int argc, char *argv[])
{
    ...
    uintptr_t g_world = 0x0;
    if (Utils::is_package_running("com.garena.game.df"))
    {
        target_pid.store(Utils::find_pid_by_package_name("com.garena.game.df"));
        g_world = 0x1ad79b68;
    }
    else if (Utils::is_package_running("com.proxima.dfm"))
    {
        target_pid.store(Utils::find_pid_by_package_name("com.proxima.dfm"));
        g_world = 0x1ad84e78;
    }
    else
    {
    ...
    while (is_running.load(std::memory_order_relaxed))
    {
        ...
        uintptr_t u_world = Memory::Read<uintptr_t>(lib_base + g_world, target_pid);
        ...
        uintptr_t cache_cur_weapon_local = Memory::Read<uintptr_t>(acknowledged_pawn + Offset::cache_cur_weapon, target_pid);
        ...
        for (size_t i = 0; i < player_array.count; i++)
        {
            ...
            bool is_alive = current_health > 0;
            if (!is_alive)
                continue;
            if (current_health < 0)
                continue;
            ...
            // if (aim_is_aiming.load() && aim_is_weapon_firing.load() && is_on_screen && is_alive && screen_distance <= aim_fov.load())
            // {
